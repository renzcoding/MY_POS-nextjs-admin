...
export default function SignInForm() {
  ...
  const callbackUrl: any = useSearchParams().get("callbackUrl") || "/admin";
    ...
    try {
      ...
      */
      ...
      if (response?.error) {
        ...
        // push(`/auth/error?error=${encodeURIComponent(response?.error)}`);
      } else {
        setLoading(false);
        setTimeout(() => {
          e.target.reset();
      ...
      <div className="mx-auto flex w-full max-w-md flex-1 flex-col justify-center">
        <div>
          ...
          <div>
            ...
            {error && (
              <p className="text-error-500 my-5 text-center text-sm">{error}</p>
            )}
