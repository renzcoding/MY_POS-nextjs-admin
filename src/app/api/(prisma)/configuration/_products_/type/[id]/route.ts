...
export const PATCH = async (req: NextRequest, context: { params: any }) => {
  ...
  try {
    const existSize = await prisma.product_Type.findUnique({
      where: { id: Number(id) },
    });
    ...
    const updateProductSize = await prisma.product_Type.update({
      where: { id: Number(id) },
      data: body,
    });
...
export const DELETE = async (req: NextRequest, context: { params: any }) => {
  ...
  try {
    ...
    const deleteSize = await prisma.product_Type.delete({
      where: { id: Number(id) },
    });
